...
const Personal = () => {
    ...
    useEffect(() => {
        const fetchProfileData = async () => {
            try {
                const data:any = await ApiService.getProfileData();
                console.log('--->',data);
                setFormData({
                    name: data?.name || '',
    ...
    const handleChange = (e:any) => {
       ...
       const handleImageUpload = async (e: any) => {
        ...
        try {
            const response = await ApiService.uploadImage(formData); // Upload image
            if (response?.data?.file_url) {
                setFormData((prevData) => ({
                    ...prevData,
                    profile_pic: response.data.file_url, // Set uploaded URL to formData
                }));
    ...
    const handleSubmit = async (e:any) => {
        ...
        try {
            await ApiService.updateProfile(formData); // Call API service
                                ...
                                <Form.Group className="mb-3">
                                    ...
                                    <Flatpickr
                                        ...
                                        onChange={(date:any) =>
                                            setFormData((prevData) => ({
                                                ...prevData,
                                                dob: date[0], // Set the first selected date
                                            }))
                                        }
