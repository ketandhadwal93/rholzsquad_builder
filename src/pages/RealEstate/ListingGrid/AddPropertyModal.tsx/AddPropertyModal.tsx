...
// const AddPropertyModal = ({ show, handleClose ,fetchDashboardData}: any) => {
...
import { ToastContainer } from "react-toastify";
import { createSelector } from "reselect";
import Description from "pages/Components/bootstrapUI/Typography/Description";
import ApiService from "serviceArchitecture/services/apiservice";
// const AddUserModal = ({ show, handleClose ,fetchDashboardData}: any) => {

const AddPropertyModal = ({ show, handleClose ,fetchPropertiesData}: any) => {
    const selectRealEstateGridList = createSelector(
        (state: any) => state.RealEstate,
        (realEstate) => ({
            realestateGridList: realEstate.realEstateGridList
    ...
    const formik: any = useFormik({
        ...
        onSubmit: async (values: any) => {
            ...
            try {
                ...
                const uploadResponse = await ApiService.uploadImage(formData); // Call the image upload API
                const imageUrl = uploadResponse.data.file_url; // Assuming the image URL is in uploadResponse.data.file_url
    
                // Step 2: Send property data with uploaded image URL
                const propertyData = {
                    name: values.title,
                    description: values.description,
                ...
                await ApiService.addProperty(propertyData); // Call the add property API
                
                // Step 3: Refresh the data and reset the form
                if (fetchPropertiesData) {
                    fetchPropertiesData(); // Refresh the dashboard data if required
                }
                
                formik.resetForm(); // Reset form fields
                setSelectfeils([]); // Reset file selection
...
{loading ? 'Adding Property...' : 'Add Property'}</Button>
            </div>
        </Modal.Footer>
    </Form>
</Modal >
