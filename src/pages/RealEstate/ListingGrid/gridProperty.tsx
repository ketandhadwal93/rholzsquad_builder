...
//                                                 <i className="bi bi-geo-alt align-baseline me-1"></i>{item.description}
//                                             </p>
...
const GridProperty = ({ data ,fetchPropertiesData}: any) => {
    ...
    const formik: any = useFormik({
        ...
        enableReinitialize: true,
        initialValues: {
            id: ( realestategrid?.id) || '',
            title: (realestategrid && realestategrid.title) || '',
           
            img: (realestategrid && realestategrid.image) || '', 
           description:(realestategrid && realestategrid.description)||'',
        },
        validationSchema: Yup.object({
            title: Yup.string().required("Please Enter Your  Title"),
            description: Yup.string().required("Please Enter Your Description"), // Add validation for description
...
setLoading(true);
            try {
                ...
                if (values.img && values.img instanceof File) {
                    ...
                    // Replace this with your image upload API call
                    const response = await ApiService.uploadImage(formData); 
                    imageUrl = response.data.file_url; // Adjust according to your API response
                }
    console.log(values)
                const updateData = {
                    // id: realestategrid?.id,
                ...
                // Dispatch the action to update the property
                await ApiService.updateProperty(values.id, updateData);
    
                formik.resetForm();
                if(fetchPropertiesData){
                    fetchPropertiesData()
                }
                handleEditClose(); // Close the modal after successful submission
      ...
      {loading ? (
        ...
        <Grid container spacing={3}>
          {Array.isArray(currentdata) && currentdata?.length > 0 ? (
            ...
            <Modal show={editProperty} onHide={handleEditClose}>
                ...
                <Form className="tablelist-form" autoComplete="off" onSubmit={formik.handleSubmit}>
                    <Modal.Body>
                                            ...
                                            <li className="mt-2 dz-image-preview" key="">
                                                <div className="border rounded">
                                                    <div className="d-flex flex-wrap gap-2 p-2">
                                                        <div className="flex-shrink-0 me-3">
                                                            <div className="avatar-sm bg-light rounded p-2">
                                                                <img data-dz-thumbnail="" className="img-fluid rounded d-block" src={file?.priview || file.image } alt={file?.name || ''} />
                                                            </div>
                                                        </div>
                                                        <div className="flex-grow-1">
                                                            <div className="pt-1">
                        ...
                        <div className="mb-3">
                <Form.Label htmlFor="property-title-input">Property Description<span className="text-danger">*</span></Form.Label>
                <Form.Control
                        as="textarea"
                    id="description"
                    rows={3}
            ...
            <DeleteModal show={show} handleClose={handleDeleteClose} deleteModalFunction={deleteModalFunction}  title="Are you sure you want to delete this property?"
