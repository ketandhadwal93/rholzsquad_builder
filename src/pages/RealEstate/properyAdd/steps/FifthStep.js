  ...
  const handleImageChange = async (event, type) => {
    try {
      ...
      for (const file of files) {
        ...
        // let apiImageRes = ApiService.uploadImage('file', file);

        const formData = new FormData();
        formData.append('file', file); // Ensure 'file' is the correct field name expected by the API

        console.log("Uploading file:", file);

        // Call the API to upload the image using FormData
        const apiImageRes = await ApiService.uploadImage(formData); // Assuming the ApiService method accepts FormData
